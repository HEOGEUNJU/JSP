 ...
 *   	 : web.cml -> servlet -> servlet-name, servlet-class => 서블릿 스펙(2.X)대 버전
 *   	 : @Webservlet => 서블릿 스펙(3.X)대 버전
 *   5. 서블릿 매핑 설정 
 *   	 : web.xml -> servlet-mapping -> servlet-name, url-pattern => 서블릿 스펙(2.X)대 버전
 *   	 : @Webservlet(속성들) => 서블릿 스펙(3.X)대 버전 
 *   6. 컨테이너 재구동
 *   
 *   ** servlet container ?
 * 		 : 컨테이너에 의해 관리되는 서블릿 객체의 생명주기 제어.
 * 	 서블릿 객체의 싱글턴 인스턴스 생성 : 구체적인 설정(loadOnStartup = ?(생성순서))이 없는 한, 매핑된 조건의 요청이 발생했을 때 생성
 * 
 * 	 ** 서블릿에서 재정의할 수 있는 콜백 메소드 종류.
 *	 생명주기 이벤트
 *		생성 : init
 *		소멸 : destroy
 *	 요청관련 이벤트 : service, doXXX
 *	 
...
public class DescriptionServlet extends HttpServlet{
	...
	@Override//service의 super는 do계열의 메소드를 실행 함
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		resp.getWriter().println(String.format("decription-%s", Thread.currentThread().getName()));
