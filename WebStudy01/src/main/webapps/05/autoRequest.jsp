...
<head>
<meta charset="UTF-8">
<!-- <meta http-equiv="Refresh" content="5;url=https://www.naver.com" > --><!--  서버사이드 방식으로 설정 -->
...
<pre>
   ...
   1. Refresh response header ->화면에 락을 걸어야하는 구조
   (cf: 비동기는 화면에 락을 걸 수 없음)
   2. meta tag
   3. reload
</pre>
...
<button class= "controlBtn disabled" data-control-type="STOP" >멈춤</button><br><br>
<input type="radio" name="dataType" data-data-type="json"/>JSON
<input type="radio" name="dataType" data-data-type="text"/>PLAIN<br><br>
...
// 			컨트롤버튼 타입이 START면 시계 작동
			jobId=setInterval(sendRequest, 1000);
			timeArea.data("jobId",jobId);
			
		}else{
// 			else 컨트롤버튼 타입이 STOP이면 시계 멈추기 
