...
*사용빈도 수에 따른 정리*
	...
	out(JspWriter) :== response.getWriter(), response body 에 컨텐츠를 기록(버퍼를 제어)할 때 활용
	session(HttpSession) : 한 클라이언트와 하나의 브라우저를 대상으로 생성되는 한 세션에 대한 정보를 캡슐화한 객체.
	application(SerlvetContext) : 하나의 컨텍스트와 서버에 대한 정보를 캡슐화한 객체.
	
	page(Object) == this, custom tag 작성시 활용됨.
	config(ServletConfig) : 현재 서블릿의 설정 정보를 캡슐화한 객체.
	
	pageContext(PageContext *****제일 중요) : 현재 JSP페이지에 대한 모든 정보를 캡슐화한 객체.	
	ex) el태그 ${pageContext.request.contextPath } 에서 많이 활용 
