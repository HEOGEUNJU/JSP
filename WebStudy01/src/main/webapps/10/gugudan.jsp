...
(2*3=6)
2. 세번째 row의 배경색을 빨간색으로 출력. (inlince css 속성 사용하지 않도록.)
3. 맨 마지막 row의 배경색 초록색으로  
-->
...
<form>
   <select name="min">
      <c:forEach var="dan1" begin="2" end="9" varStatus="vs" >
      	<option value="${dan1 }"${dan1 eq minDan ? "selected":"" }> ${dan1 }단</option>
      </c:forEach>
   </select>
   
   <select name="max">
      <c:forEach var="dan2" begin="2" end="9" varStatus="vs" >
      	<option value="${dan2 }" ${dan2 eq minDan ? "selected":"" }> ${dan2 }단</option>
      </c:forEach>
   </select>
