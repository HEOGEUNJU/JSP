...
<pre>
	: 커스텀 태그들 중에서 많이 활용될 수 있는 것들을 모아 놓은 라이브러리.
	...
	** core 태그 종류
	...
	3. 반복문 : foreach, forTokens, for(선언절, 조건절, 증감절) for(임시 블럭 변수 : 반복할 대상 집합 객체)
...
<%-- 			<c:redirect url="/06/memoView.jsp"></c:redirect> --%>
			...
			--%>
		표현구조 : out -> escape를 자동으로 시켜줌
<%-- 			<jsp:include page=""></jsp:include> --%>
<%-- 			<c:import url="http://www.naver.com"></c:import> <!-- 컨텍스트에 제한이 없어서 include는 naver임포트가 안되는데 c는 가능 --> --%>
			<c:out value="<h4>출력값</h4>" escapeXml="false"/>
			
</pre>
<c:import url="https://www.naver.com" var ="naver"/> <!-- 컨텍스트에 제한이 없어서 include는 naver임포트가 안되는데 c는 가능 -->
