 ...
 *   
 *   Container란? 객체(bean)의 생명주기 관리자, 객체의 생성(소멸), 주입구조를 통한 의존관계 형성.
 *   
 *   사용 단계 
 *   1. Spring bean configuration file : 
 *   		bean 엘리먼트로 객체 등록
 *   2. 등록된 bean 들간의 의존 관계 형성(주입 구조)
 *   		- 생성자 주입(constructor injection) - constructior -arg
 ...
 *   4. 필요한 객체(bean)를 주입 - getBean
 *   
 *   컨테이너 빈 관리 정책
 *   1. 특별한 설정(scope) 없는 한 등록된 모든 빈은 싱글턴으로 관리.
 *   		singleton : 빈 하나당 객체가 하나씩 생성 및 관리. 주입 횟수와 상관 없이 하나의 객체가 공유.
 *   		prototype : 주입될 때마다 새로운 객체가 생성 및 관리됨. 객체의 생성 시점이 지연됨.
 *   		request/session(web 전용) : requestScope, sessionScope의 생명주기와 동일해짐.
 *   2. 특별한 설정이 없는 한 컨테이너가 초기화 될 때 모든 객체 동시 생성
 *   3. 빈들의 객체 생성 순서(depends-on)를 제어할 수 있는 설정이 있음.
 *   4. 컨테이너는 빈의 생명 주기를 관리할 때 callback 구조를 활용함.
