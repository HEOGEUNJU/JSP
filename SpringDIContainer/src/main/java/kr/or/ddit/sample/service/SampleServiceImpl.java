...
public class SampleServiceImpl implements SampleService, ApplicationContextAware {
   ...
   public void setApplicationContext(ApplicationContext arg0) throws BeansException {
      ...
      log.info("setApplicationContextAware 구조로 컨테이너 주입됨.");
   }
...
//   private SampleDAO dao = new SampleDAOImpl_Postgre();
   ...
   // case2 : Factory Object[Method] Pattern (Factory객체가 결합력 전부 받음)
//   private SampleDAO dao = SampleDAOFactory.getSampleDAO();

   // case3 : Strategy Pattern, 전략 주입자가 필요함. (전략 주입자가 결합력 전부 받음)
...
//   @Autowired
   ...
   public SampleServiceImpl(SampleDAO dao) {
      super();
      this.dao = dao;
      log.info("{} 객체 생성 및 전략 객체({})", getClass().getSimpleName(), dao.getClass().getSimpleName());
   }
   ...
   public void setDao(SampleDAO dao) {
      this.dao = dao;
      log.info("객체 생성 후 setter를 통해 주입받음", getClass().getSimpleName(), dao.getClass().getSimpleName());
   }
